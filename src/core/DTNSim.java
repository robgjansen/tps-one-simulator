...
public class DTNSim {
	/**
	 * If this option ({@value} ) is given to program, batch mode and Text UI
	 * are used
	 */
	...
	/**
	 * Starts the user interface with given arguments. If first argument is
	 * {@link #BATCH_MODE_FLAG}, the batch mode and text UI is started. The
	 * batch mode option must be followed by the number of runs, or a with a
	 * combination of starting run and the number of runs, delimited with a
	 * {@value #RANGE_DELIMETER}. Different settings from run arrays are used
	 * for different runs (see {@link Settings#setRunIndex(int)}). Following
	 * arguments are the settings files for the simulation run (if any). For GUI
	 * mode, the number before settings files (if given) is the run index to use
	 * for that run.
	 * 
	 * @param args
	...
	/**
	 * Registers a class for resetting. Reset is performed after every batch run
	 * of the simulator to reset the class' state to initial state. All classes
	 * that have static fields that should be resetted to initial values between
	 * the batch runs should register using this method. The given class must
	 * have a static implementation for the resetting method (a method called
	 * {@value #RESET_METHOD_NAME} without any parameters).
	 * 
	 * @param className
	 *            Full name (i.e., containing the packet path) of the class to
	 *            register. For example: <code>core.SimClock</code>
	 */
	...
	private static int[] parseNrofRuns(String arg) {
		...
		try {
			if (arg.contains(RANGE_DELIMETER)) {
				val[0] = Integer.parseInt(arg.substring(0, arg
