 ...
 * need host2Id field too (the host who the message is/was being transferred
 * to). CREATE action needs the additional size (of the message) field and can
 * have also size-of-the-response field if a response to this message is
 * requested.
 * </P>
 */
		...
		while (eventsRead < nrof && scanner.hasNextLine()) {
			...
			try {
				...
				if (action.equals(DROP)) {
					...
					events.add(new MessageDeleteEvent(hostAddr, msgId, time,
							true));
