 ...
 * @author Frans Ekman
 */
public class HomeActivityMovement extends MapBasedMovement implements
		SwitchableMovement {
	...
	public HomeActivityMovement(Settings settings) {
		...
		try {
			homeLocationsFile = settings
					.getSetting(HOME_LOCATIONS_FILE_SETTING);
		} catch (Throwable t) {
			// Do nothing;
		}
		...
		if (homeLocationsFile == null) {
			MapNode[] mapNodes = (MapNode[]) getMap().getNodes().toArray(
					new MapNode[0]);
			...
			try {
				...
				List<Coord> locationsRead = (new WKTReader())
						.readPoints(new File(homeLocationsFile));
				...
				homeLocation = allHomes.get(rng.nextInt(allHomes.size()))
						.clone();
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		if (timeDiffSTD == -1) {
			...
			timeDifference = (int) Math.min(Math.max(
					(rng.nextGaussian() * timeDiffSTD), -DAY_LENGTH / 2),
					DAY_LENGTH / 2);
		}
	}
	...
	protected double generateWaitTime() {
		if (mode == AT_HOME_MODE) {
			return DAY_LENGTH
					- ((SimClock.getIntTime() + DAY_LENGTH + timeDifference) % DAY_LENGTH);
		} else {
			return 0;
		}
	}
