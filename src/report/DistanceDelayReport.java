...
/**
 * Report for how far apart the nodes were when the message was sent and how
 * long time & how many hops it took to deliver it. Only messages created after
 * the warm up period are counted. If message is not delivered, its delivery
 * time & hop count are reported as -1
 */
public class DistanceDelayReport extends Report implements MessageListener {
	...
	public void messageTransferred(Message m, DTNHost from, DTNHost to,
		...
		report(m.getId(), info.getLoc1().distance(info.getLoc2()), getSimTime()
				- info.getTime(), m.getHops().size() - 1);
	}
	...
	public void newMessage(Message m) {
		...
		// we might not know the "to" location of anon messages
		Coord toLoc = null;
		if(m.getTo() != null){
			toLoc = m.getTo().getLocation().clone();
		} else{
			toLoc = new Coord(Double.NaN, Double.NaN);
		}
		this.creationInfos.put(m.getId(), new InfoTuple(getSimTime(), m
				.getFrom().getLocation().clone(), toLoc));
	}
