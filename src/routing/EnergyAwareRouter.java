 ...
 */
public class EnergyAwareRouter extends ActiveRouter implements
	 ...
	 * 
	 * @param range
	 *            The min and max values of the range, or if only one value is
	...
	protected void setEnergy(double range[]) {
		if (range.length == 1) {
			...
			this.currentEnergy = range[0] + rng.nextDouble()
					* (range[1] - range[0]);
		}
	}
