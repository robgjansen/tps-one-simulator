...
import report.AnonymousGroupReport;
...
public abstract class DTNSimUI {
	/**
	 * Number of reports -setting id ({@value} ). Defines how many reports are
	 * loaded.
	 */
	public static final String NROF_REPORT_S = "Report.nrofReports";
	/**
	 * Report class name -setting id prefix ({@value} ). Defines name(s) of the
	 * report classes to load. Must be suffixed with numbers starting from one.
	 */
	 ...
	 */
	public static final String MM_WARMUP_S = movement.MovementModel.MOVEMENT_MODEL_NS
			+ ".warmup";
	...
	private void initModel() {
		...
		try {
			...
			// define groups if setting exists
			AnonymousGroupManager.defineGroups();
	...
	public void done() {
		for (Report r : this.reports) {
			if(r instanceof AnonymousGroupReport){
				((AnonymousGroupReport)r).setHosts(this.world.getHosts());
			}
			r.done();
		}
		//reset and clear the defined groups for next sim run
		AnonymousGroupManager.resetGroups();
	}
