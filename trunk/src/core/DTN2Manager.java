...
/**
 * Manages the external convergence layer connections to dtnd. Parses the
 * configuration file and sets up the CLAParsers and EID->host mappings.
 * 
 * @author teemuk
 */
public class DTN2Manager {
	...
	/**
	 * Sets up the dtnd connections by parsing the configuration file defined in
	 * the <code>DTN2.configFile</code> setting.
	 * 
	 * @param world
	...
	public static void setup(World world) {
		...
		while (s != null) {
			...
			if (attrs.length == 5 && !s.startsWith("#")) {
				 ...
				 */
				DTN2Events.ParserHandler ph = DTN2Manager.events
						.getParserHandler(nodeID, dtnd_host, console_port);
				p.setListener(ph);
	 ...
	 * to dtnd. This should be used by the dynamically loaded DTN2Reporter
	 * object to allow other objects get reference to it.
	 * 
	 * @param reporter
	...
	/**
	 * Returns a <code>Collection</code> of <code>DTNHost</code> objects
	 * corresponding to the given EID.
	 * 
	 * @param EID
	 *            EID of the host
