...
/**
 * Intermodule communication bus. Works as a blackboard where modules can post
 * data, subscribe to data changes and also poll for data values. This is fairly
 * similar to Message class' property interface, but these values are shared for
 * a node instead of message.
 */
public class ModuleCommunicationBus {
	...
	/**
	 * Adds a new property for this node. The key can be any string but it
	 * should be such that no other class accidently uses the same value. Note
	 * that, unless the value is immutable, it can be changed by any object that
	 * can call {@link #getProperty}.
	 * 
	 * @param key
	...
	/**
	 * Returns an object that was stored using the given key. If such object is
