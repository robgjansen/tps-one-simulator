 ...
 */
public class EventLogPanel extends JPanel implements ConnectionListener,
	 ...
	 */
	private void addEvent(String description, DTNHost host1, DTNHost host2,
			Message message) {
		JPanel eventPane = new JPanel();
		eventPane.setLayout(new BoxLayout(eventPane, BoxLayout.LINE_AXIS));

		String text = String.format(ENTRY_FORMAT, SimClock.getTime(),
				description);
	...
	private void processEvent(EventLogControl check, final String name,
		...
		descString = name + " " + (host1 != null ? host1 : "")
	...
	public void messageTransferred(Message m, DTNHost from, DTNHost to,
		...
		if (firstDelivery) {
			...
			processEvent(msgDeliveredCheck, "Message delivered again", from,
					to, m);
		} else {
	...
	public void messageTransferStarted(Message m, DTNHost from, DTNHost to) {
		processEvent(msgTransferStartCheck, "Message relay started", from, to,
				m);
