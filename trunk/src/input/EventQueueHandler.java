 ...
 * queues: external event queues and event generator classes. For external event
 * queues, the events are defined in external data file(s) (see e.g.
 * input.StandarEventsReader). Event generator classes define events
 * dynamically. Both type of event queues must implement the input.EventQueue
 * interface.
 * <p>
 ...
 * <p>
 * External event files are used when the variable <code>PATH_SETTING</code> is
 * used to define the path to the event file and event generator class is loaded
 * when the name of the class is defined with <code>CLASS_SETTING</code>.
 */
	...
	public EventQueueHandler() {
		...
		for (int i = 1; i <= nrof; i++) {
			...
			if (s.contains(PATH_SETTING)) { // external events file
				...
				EventQueue eq = (EventQueue) s
						.createIntializedObject(className);
