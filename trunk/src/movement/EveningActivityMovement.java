 ...
 * @author Frans Ekman
 */
public class EveningActivityMovement extends MapBasedMovement implements
		SwitchableMovement {
	...
	public EveningActivityMovement(Settings settings) {
		...
		if (shoppingSpotsFile == null) {
			...
			try {
				...
				List<Coord> locationsRead = (new WKTReader())
						.readPoints(new File(shoppingSpotsFile));
		...
		scs = EveningActivityControlSystem
				.getEveningActivityControlSystem(scsID);
	...
	public Path getPath() {
		if (mode == WALKING_TO_MEETING_SPOT_MODE) {
			...
			if (trip.allMembersPresent()) {
				...
				if (path == null) {
					super.lastMapNode = super.getMap().getNodeByCoord(
							lastWaypoint);
