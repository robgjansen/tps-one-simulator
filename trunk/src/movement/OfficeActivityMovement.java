...
public class OfficeActivityMovement extends MapBasedMovement implements
	...
	public OfficeActivityMovement(Settings settings) {
		...
		officeWaitTimeParetoCoeff = settings
				.getDouble(OFFICE_WAIT_TIME_PARETO_COEFF_SETTING);
		...
		try {
			officeLocationsFile = settings
					.getSetting(OFFICE_LOCATIONS_FILE_SETTING);
		} catch (Throwable t) {
			// Do nothing;
		}
		...
		if (officeLocationsFile == null) {
			...
			try {
				...
				officeLocation = allOffices.get(rng.nextInt(allOffices.size()))
						.clone();
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public Coord getRandomCoorinateInsideOffice() {
		double x_coord = officeLocation.getX() + (0.5 - rng.nextDouble())
				* distance;
		if (x_coord > getMaxX()) {
			x_coord = getMaxX();
		} else if (x_coord < 0) {
		...
		double y_coord = officeLocation.getY() + (0.5 - rng.nextDouble())
				* distance;
		if (y_coord > getMaxY()) {
			y_coord = getMaxY();
		} else if (y_coord < 0) {
