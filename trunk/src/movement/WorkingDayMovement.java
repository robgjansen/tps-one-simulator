 ...
 * movement with daily routines. People wake up in the morning, go to work, go
 * shopping or similar activities in the evening and finally go home to sleep.
 * 
 * @author Frans Ekman
 */
public class WorkingDayMovement extends ExtendedMovementModel {
	...
	public WorkingDayMovement(Settings settings) {
		...
		doEveningActivityProb = settings
				.getDouble(PROBABILITY_TO_GO_SHOPPING_SETTING);
		...
		case WORK_MODE:
			if (workerMM.isReady()) {
				...
				if (doEveningActivityProb > rng.nextDouble()) {
					movementUsedForTransfers.setNextRoute(workerMM
							.getOfficeLocation(), eveningActivityMovement
							.getShoppingLocationAndGetReady());
					mode = BUS_TO_EVENING_ACTIVITY_MODE;
				} else {
					movementUsedForTransfers.setNextRoute(workerMM
							.getOfficeLocation(), homeMM.getHomeLocation());
