...
/**
 * Tests for EpidemicRouter and, due the simple nature of Epidemic router, also
 * ActiveRouter in general.
 */
public class EpidemicRouterTest extends AbstractRouterTest {
	...
	/**
	 * try disconnecting on the same update interval when a transfer should be
	 * finished -> should not cause abort (anymore)
	 */
	...
	public void testQueueManagement() {
		...
		assertEquals(msgId1, h1.getMessageCollection().iterator().next()
				.getId());
	...
	public void testResponse() {
		...
		assertTrue(mc.next());
		assertEquals(mc.TYPE_CREATE, mc.getLastType());
		assertEquals(routing.ActiveRouter.RESPONSE_PREFIX + msgId1, mc
				.getLastMsg().getId());
		assertEquals(h3, mc.getLastFrom());
		assertEquals(h1, mc.getLastTo());
	 ...
	 *            If true, the other node is the final destination of the
	 *            messages, if false, the other node is not the final dst
