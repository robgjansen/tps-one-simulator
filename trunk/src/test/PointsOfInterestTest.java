...
/**
 * Test for selecting Points Of Interest from different node groups. Tests that
 * the probability of getting a node from a specific group is approximately same
...
public class PointsOfInterestTest extends TestCase {
	...
	private static final Coord[][] COORDS_IN_MAP = {
			...
			{ new Coord(4, 1), new Coord(1, 2) }, { new Coord(3, 1) } };
	 ...
	 */
	public void runTestPoiSelection(double[] poiProbs, int rngSeed)
			throws Exception {
		...
		for (int i = 0; i < nrofHits.length; i++) {
			...
			assertEquals("Prob too far for index " + i, expectedProbs[i], prob,
					DELTA);
		}
